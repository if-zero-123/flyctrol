...
althold req=1 ready=1 active=0 baro_ok=1 imu_ok=1 tilt_ok=1 armed=0 alt=8cm vel=0cm/s out=0 corr=0 takeoff_thr=560 state=1 stick_ref=500 imu_pred=1 reliable=1 reject=0 limit=100 base=80 hover=420 target_vel=0 est_vel=0 freeze=0 bq=100 sat=0,0 scale=1000 thr_ok=0 thr_min=80
...
baro hold active=0 velctl=0 hold=0cm err=0cm target_vel=0cm/s est_vel=0cm/s base=80 hover=420 corr=0 out=0 state=1 stick_ref=500 imu_pred=1 reliable=1 reject=0 limit=100 freeze=0 bq=100 sat=0,0 scale=1000
...
class ParserTest(unittest.TestCase):
    def test_parse_sample_cli_output(self) -> None:
        ...
        self.assertEqual(state["althold"]["correction_limit"], 100)
        self.assertEqual(state["althold"]["base"], 80)
        self.assertEqual(state["althold"]["hover"], 420)
        self.assertEqual(state["althold"]["freeze_reason"], 0)
